...
export function StickyMobileBar() {
  return (
    <div className="fixed bottom-0 left-0 right-0 md:hidden z-50 grid grid-cols-2 border-t border-[var(--color-border)] bg-white shadow-[0_-4px_20px_rgba(15,23,42,0.10)]">
      <a
        href={`https://wa.me/${WA_NUMBER}`}
        target="_blank"
        rel="noopener noreferrer"
        className="py-4 flex items-center justify-center gap-2 bg-[var(--color-whatsapp)] text-white font-semibold text-sm"
      >
      ...
      <a
        href={`tel:${TEL_NUMBER}`}
        className="py-4 flex items-center justify-center gap-2 bg-[var(--color-action)] text-white font-semibold text-sm"
      >
        <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
